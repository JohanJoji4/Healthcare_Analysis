...
SELECT Chain_Organization as `Chain Organization`,
...
FROM `health care` group by `Chain Organization` order by `Total Profit` desc;


-- kpi3
-- Chain Organizations w.r.t. Total Performance Score as No Score

SELECT Chain_Organization as `Chain Organization`, Count(Total_Performance_Score) AS `Total Performance Score`
FROM `health care` where Total_Performance_Score= '0' group by `Chain Organization` order by `Total Performance Score` desc ;


-- kpi4
-- # of_Dialysis_Stations stats..
select Chain_Organization as `Chain Organization`, count(Number_of_Dialysis_Stations) as `Number of Dialysis Stations` from `health care` group by `Chain Organization` 
...
SELECT Ownership_as_of_December_31_2018 as `Ownership As of 31st December 2018`, AVG(PY2020_Payment_Reduction_Percentage) AS `Average Payment Reduction Rate`
FROM `health care` group by `Ownership As of 31st December 2018` order by `Average Payment Reduction Rate` desc;
